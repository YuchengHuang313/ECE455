...
void check(cudaError_t err, const char* const func, const char* const file, int line) {
    if (err != cudaSuccess) {
        std::cerr << "CUDA Runtime Error at " << file << ":" << line << " — " << cudaGetErrorString(err) << " (" << func << ")" << std::endl;
...
std::vector<T> create_rand_vector(size_t n, T min_val = 0, T max_val = 10) {
    ...
    std::vector<T> vec(n);
    for (size_t i = 0; i < n; ++i) vec[i] = static_cast<T>(dist(e));
    return vec;
}
...
void mm_host(const T* A, const T* B, T* C, int N) {
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            double acc = 0.0;
            for (int k = 0; k < N; ++k) acc += static_cast<double>(A[i * N + k]) * static_cast<double>(B[k * N + j]);
...
__global__ void mm_tiled(const T* A, const T* B, T* C, int N) {
    ...
    for (int t = 0; t < (N + TILE_SIZE - 1) / TILE_SIZE; ++t) {
        ...
        for (int k = 0; k < TILE_SIZE; ++k) val += tile_A[threadIdx.y][k] * tile_B[k][threadIdx.x];
...
template <typename T>
bool validate_results(const std::vector<T>& ref, const std::vector<T>& gpu, int N, T rel_tol = static_cast<T>(1e-2)) {
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            size_t idx = i * N + j;
            ...
            if (diff / denom > rel_tol) {
                std::cerr << "Mismatch at (" << i << ", " << j << "): "
                          << "CPU=" << ref[idx] << ", GPU=" << gpu[idx] << ", rel_err=" << diff / denom << std::endl;
                return false;
            }
        }
    }
...
float measure_latency_mm_cuda(int N, int num_tests, int num_warmups) {
    ...
    for (int i = 0; i < num_warmups; ++i) mm_cuda<T>(d_A, d_B, d_C, N);

    checkCuda(cudaEventRecord(startEvent, 0));
    for (int i = 0; i < num_tests; ++i) mm_cuda<T>(d_A, d_B, d_C, N);
...
int main() {
    ...
    std::cout << "Average kernel runtime: " << std::fixed << std::setprecision(4) << avg_ms << " ms" << std::endl;
