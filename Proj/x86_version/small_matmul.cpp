...
int main(int argc, char** argv) {
    ...
    bool correct_gpu = compare_results(h_out_cpu, h_out_gpu, total_elements);
    bool correct_omp = compare_results(h_out_cpu, h_out_cpu_omp, total_elements);

    if (correct_gpu && correct_omp) {
        std::cout << "✓ All results match! GPU and OMP implementations are correct." << std::endl;
    } else {
        if (!correct_gpu) std::cout << "✗ GPU results DO NOT match!" << std::endl;
        if (!correct_omp) std::cout << "✗ OMP results DO NOT match!" << std::endl;
    }
