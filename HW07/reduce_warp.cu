...
void check(cudaError_t err, const char* const func, const char* const file, int line) {
    if (err != cudaSuccess) {
        std::cerr << "CUDA Error at " << file << ":" << line << " — " << cudaGetErrorString(err) << " (" << func << ")" << std::endl;
...
std::vector<int> create_rand_vector(size_t n, int min_val = 0, int max_val = 100) {
    ...
    for (size_t i = 0; i < n; ++i) vec[i] = dist(e);
    return vec;
}
...
__global__ void reduce_warp(const int* in, int* out, size_t n_elems) {
    ...
    if (threadIdx.x % WARP_SIZE == 0) atomicAdd(out, val);
}
...
int reduce_cuda_warp(const std::vector<int>& h_in, float& avg_time_ms) {
    ...
    // Warm-up
    for (int i = 0; i < num_warmups; ++i) reduce_warp<<<blocks, threads>>>(d_in, d_out, N);
